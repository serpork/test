...
export default class RecipeListItem extends Component {
  constructor(props) {
    ...
    this.state = { expand: false, editForm: '', recipeTitle: recipeArr[0], recipeContent: ingridientList };
  }
  ...
  onEditClick() {
    let formItem = (
      <form className="edit-form" onSubmit={((event) => {this.editHandler(event, this.props.storageKey);}).bind(this)}>
  ...
  render() {
    ...
    if (!this.state.expand) {
      return (
        ...
        <div className="recipe-title" onClick={this.onTitleClick.bind(this)}>
    ...
    return (
      <div className="recipe-item">
        <div onClick={this.onTitleClick.bind(this)} className="recipe-title">{this.state.recipeTitle}</div>
        <div className="form-content">
          {this.state.recipeContent}
          <div className="btn-group">
            <button className="btn-edit" onClick={this.onEditClick.bind(this)}>Edit</button>
            <button className="btn-delete" onClick={(() => { this.props.removeHandler(this.props.storageKey); }).bind(this)}>Delete</button>
          </div>
          {this.state.editForm}
        </div>
      </div>
